...
def do_organization_payment_view(request, purchase, organization):
    ...
    return render(request, 'debits_test/organization-payment-view.html',
                  {'organization_id': organization.pk,
                   ...
                   'processor_name': purchase.payment.transaction.processor.name if purchase.payment else None,  # FIXME: Does not work for subscription with delayed first payment
...
def do_upgrade(hash, form, processor, purchase, organization):
    ...
    if not purchase.subscribed:
        ...
        upgrade_transaction = SubscriptionTransaction.objects.create(processor=processor, purchase=purchase)
